...
#I pledge my honor that I have adided by the Stevens Honor System - A.P
...
def gor3(x,y,z):
    '''Or of three inputs.'''
    return x or y or z
    #return gor(gor(x,y),z) is another possible solution 
...
def FA(x,y,cin):
    ...
    return (gor(gand(x,y),gor(gand(x,y),gand(XOR(x,y),cin))),XOR(XOR(x,y),cin))
...
def fourBitAdd(xxxx,yyyy):
    ...
    (e,ze)=FA(xxxx[1],yyyy[1],d)
    (f,zf)=FA(xxxx[0],yyyy[0],e)
    return (f,(zf,ze,zt,zo))
